...
TINY.editor = function() {
	...
	function edit(n, a) {
		...
		var p = document.createElement('div'), w = document
				.createElement('div'), h = document.createElement('div'), l = a.controls.length, i = 0;
		...
		for (i; i < l; i++) {
			...
			if (b == 'n') {
				...
				var g = document.createElement('select'), fonts = a.fonts
				...
				var g = document.createElement('select'), sizes = a.sizes
				...
				var q = document.createElement('div'), x = c[b], func = x[2], ex, pos = x[0]
						* offset;
				...
				ex = func == 'a' ? '.action("' + x[3] + '",0,' + (x[4] || 0)
						+ ')' : '.insert("' + x[4] + '","' + x[5] + '","'
						+ x[3] + '")';
				q.onclick = new Function(this.n
						+ (b == 'print' ? '.print()' : ex));
				q.onmouseover = new Function(this.n + '.hover(this,' + pos
						+ ',1)');
				q.onmouseout = new Function(this.n + '.hover(this,' + pos
						+ ',0)');
				h.appendChild(q);
				if (this.ie) {
					q.unselectable = 'on'
		...
		this.e.open();
		var m = '<html><head>', bodyid = a.bodyid
				? " id=\"" + a.bodyid + "\""
				: "";
		if (a.cssfile) {
	...
	edit.prototype.print = function() {
		...
		var i = b.selectedIndex, v = b.options[i].value;
		this.action(a, v)
		...
		if (!this.d) {
			...
			if (this.xhtml) {
				...
				v = v.replace(/<span class="apple-style-span">(.*)<\/span>/g,
						'$1');
				...
				v = v
						.replace(
								/<(b|strong|em|i|u) style="font-weight: normal;?">(.*)<\/(b|strong|em|i|u)>/g,
								'$2');
				v = v
						.replace(
								/<(b|strong|em|i|u) style="(.*)">(.*)<\/(b|strong|em|i|u)>/g,
								'<span style="$2"><$4>$3</$4></span>');
				v = v.replace(
						/<span style="font-weight: normal;?">(.*)<\/span>/g,
						'$1');
				v = v.replace(
				...
				v = v
						.replace(
								/<span style="font-weight: bold;?">(.*)<\/span>|<b\b[^>]*>(.*?)<\/b[^>]*>/g,
								'<strong>$1</strong>')
			}
			if (c) {
